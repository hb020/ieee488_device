 ...
 *
 * @return The current time in microseconds since startup.
 */
static __attribute__((always_inline)) uint32_t now_us(void) { return TIME_US(); }

/** @brief Get the current time in milliseconds.
 *
 * @return The current time in milliseconds since startup.
 */
static __attribute__((always_inline)) uint32_t now_ms(void) { return TIME_MS(); }

// handshake timer: uses handshake_timeout_us.
// When 0: infinite timeout, no software timeout.
// When non-zero: the time by which the current handshake operation must complete, in microseconds.
...
/** @brief Initialize the IEEE 488 device.
 */
void ieee488_init(void) {
    memset(&ieee488_device, 0, sizeof(ieee488_device));
    ieee488_device.cfg = &cfg;
    ieee488_device.cb = &cb;
    hal_init();
    ieee488_reset(true);
}
...
static void acceptor(bool atn) {
    ...
    switch (ah) {
        ...
        case IEEE488_AH_ANRS:
            /* IEEE 488.1: ANRS to ACRS transition requires external ready state */
            rdy = (atn ||  // if atn, we need to do decode command. No delay for my use case
                   (ieee488_device.cb->rx_ready && ieee488_device.cb->rx_ready()) ||
                   !ieee488_device.cb->rx_ready);  // if no callback, assume ready
            ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
                if (ieee488_device.restart_loop) return;
                NDAC_ASSERT();
                NRFD_RELEASE();
                if (rdy) ieee488_device.ah = IEEE488_AH_ACRS;
                // arm_handshake(); // no timing in ACRS
            }
            break;
        ...
        case IEEE488_AH_ACDS:
            if (!dav) {
                ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
                    if (ieee488_device.restart_loop) return;
                    ieee488_device.ah = IEEE488_AH_ANRS; // go via the ready check. I should go to ACRS, and then if not ready, go to ANRS. This is just easier.
                }
            } else if (expired_handshake()) {
                acceptor_force_idle();
            }
            break;
